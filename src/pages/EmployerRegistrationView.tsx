...
import { CompanyStatus } from "../services/hiredInTypes";

export default function EmployerRegistrationView() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const auth = await hiredInApi.registerEmployer({
        firstName: normalizedCompanyName || "Company",
        lastName: "Employer",
        email,
        password,
      });
      ...
      if (auth.user) {
        auth.user.fullName =
          normalizedCompanyName || auth.user.fullName || email;
      }

      authContext.login(auth);

      await hiredInApi.createCompany({
        name: normalizedCompanyName || "Company",
        description: null,
        industry: null,
        city: null,
        website: null,
      ...
      navigate("/companyCabinet");
    } catch {
      setMessage(
        isEnglish
          ? "Could not create a company account. Check the data or try another email."
          : "Не вдалося створити акаунт компанії. Перевір дані або спробуй інший email.",
      );
    } finally {
          ...
          <div className="relative z-10 flex flex-1 flex-col justify-center py-6">
            <div className="max-w-[490px]">
              ...
              <p className="mb-8 text-[14px] leading-relaxed text-[#A8B6CD] lg:text-[15px]">
                {isEnglish
                  ? "Create a company account, publish vacancies, and receive applications directly from specialists."
                  : "Створи кабінет компанії, публікуй вакансії та отримуй відгуки напряму від спеціалістів."}
              </p>
            </div>
          </div>
              ...
              >
                <label>
                  ...
                  <input
                    ...
                    required
                    type="text"
                    placeholder={isEnglish ? "Company name" : "Назва компанії"}
